...
public class Locomotion
{
    ...
    private void GroundCheck()
    {
        ...
        Vector3 extents = Vector3.one * (_collider.radius * 1.5f);
        extents.y = 0f;
        //if(Physics.Raycast (transform.position + _collider.center, Vector3.down, out RaycastHit hit, 100f, mask, QueryTriggerInteraction.Ignore))
        if (Physics.BoxCast(start, extents, Vector3.down, out RaycastHit hit, _transform.rotation, 100f, mask, QueryTriggerInteraction.Ignore))
        {
            ...
            IsGrounded = hit.distance < (_collider.center.y + 1.1f);
        }
        else
            IsGrounded = false;
    }
